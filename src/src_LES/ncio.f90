   ...
   SUBROUTINE define_nc(ncID, nRec, nVar, fields, n1, n2, n3, &
         ...
         DO n = 1, nVar
            ...
            IF (ArrEl%outputstatus) THEN
               ...
               SELECT CASE(Arrel%dimension)
                  ...
                  CASE ('time')
                     iret = nf90_def_var(ncID,sx,NF90_FLOAT,timeID  ,VarID)
                  CASE ('zt')
                  ...
                  CASE ('ttica')
                     iret=nf90_def_var(ncID,sx,NF90_FLOAT,dim_ttica,VarID)
                  CASE ('tticb')
                     iret=nf90_def_var(ncID,sx,NF90_FLOAT,dim_tticb,VarID)
                  CASE ('ttsnw')
                     iret=nf90_def_var(ncID,sx,NF90_FLOAT,dim_ttsnw,VarID)
                  ...
                  CASE ('ttztica')
                     iret=nf90_def_var(ncID,sx,NF90_FLOAT,dim_ttztica,VarID)
                  CASE ('ttzticb')
                     iret=nf90_def_var(ncID,sx,NF90_FLOAT,dim_ttzticb,VarID)
                  CASE ('ttztsnw')
                     iret=nf90_def_var(ncID,sx,NF90_FLOAT,dim_ttztsnw,VarID)
      ...
      ELSE
         ...
         DO i = 1, nVar
            ...
            write(*,*) i,sx,nnum,Arrel%dimension
         END DO
   ...
   CHARACTER (len=80) FUNCTION ncinfo(itype,short_name)
      ...
      SELECT CASE (trim(short_name))
         ...
         CASE('S_Nibb')
            ...
            IF (itype == 1) ncinfo = 'm^-3'
            IF (itype == 2) ncinfo = 'tttticb' !wass ica, assuming incorrect
         CASE('S_Rwia')
         ...
         CASE('S_Rwiba')
            ...
            IF (itype == 1) ncinfo = 'm'
            IF (itype == 2) ncinfo = 'ttttica' !used to be ttttcla, assuming incorrect
         CASE('S_Rwibb')
            IF (itype == 0) ncinfo = 'SALSA bin ice particle radius, regime b'
            IF (itype == 1) ncinfo = 'm'
            IF (itype == 2) ncinfo = 'tttticb' !used to be ttttcla, assuming incorrect
         CASE('S_Ns')
