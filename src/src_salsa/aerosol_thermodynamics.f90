...
CONTAINS
   ...
   SUBROUTINE inorganic_pdfite(rh,temp,ions,water_total,Press_HNO3,Press_HCl,Press_NH3,gamma_out,mols_out)
      ...
      IF (ions(1) > 0.0e0 .AND. ions(4) > 0.0e0) THEN
         ...
         IF(gamma_H2SO4>0e0) gamma_out(4) = (gamma_HHSO4**2e0)/gamma_H2SO4
         IF(gamma_HHSO4>0e0) gamma_out(5) = (gamma_H2SO4**3e0)/(gamma_HHSO4**2e0)
         ...
         IF (root2 > ions(1) .OR. root2 < 0.0e0) THEN
            root2=0.0e0
         END IF
      ...
      ELSE IF (ions(1) == 0.0e0 .OR. ions(4) == 0.0e0) THEN
